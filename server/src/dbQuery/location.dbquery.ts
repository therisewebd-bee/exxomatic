...
import { CreateLocationLogInput, FindLocationQueryInput, LocationIdParam } from "../dto/location.dto.js";
...
const createLocationLog = catchService(async (locationDataScheam: CreateLocationLogInput) => {
    return await prismaAdapter.locationLog.create({
        data: {
            ...locationDataScheam.body
        }
    })
}, "DB-Call:Location", "Create Location Log");

const findLocationLogs = catchService(async (findLDS: FindLocationQueryInput) => {
...
}, "DB-Call:Location", "Find Location Logs");

const findLocationLogById = catchService(async (lID: LocationIdParam) => {
    return await prismaAdapter.locationLog.findUnique({
        where: {
            id: lID.params.locationId
        }
    })
}, "DB-Call:Location", "Find Location Log By Id");

const deleteLocationLog = catchService(async (lID: LocationIdParam) => {
